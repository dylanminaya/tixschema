...
/// `suspend`-method-per-operation client — published only where the `kotlin` feature publishes the
/// Kotlin types and codec this client's messages, successes and errors are written in.
#[cfg(feature = "kotlin")]
